...
`)}},"elements"),c&&s.arrayBracketSpacing?r.push(" ]"):r.push("]"),t.typeAnnotation&&r.push(i.call(e,"typeAnnotation")),x(r)}case"SequenceExpression":return S(", ").join(i.map(e,"expressions"));case"ThisExpression":return S("this");case"Super":return S("super");case"NullLiteral":return S("null");case"RegExpLiteral":return S(dt(t)||`/${t.pattern}/${t.flags||""}`,s);case"BigIntLiteral":return S(dt(t)||t.value+"n",s);case"NumericLiteral":return S(dt(t)||t.value,s);case"DecimalLiteral":return S(dt(t)||t.value+"m",s);case"StringLiteral":return S(_t(t.value,s));case"BooleanLiteral":case"Literal":return S(dt(t)||(typeof t.value=="string"?_t(t.value,s):t.value),s);case"Directive":return i.call(e,"value");case"DirectiveLiteral":return S(dt(t)||_t(t.value,s),s);case"InterpreterDirective":return S(`#!${t.value}
...
while`),r.push(" (",i.call(e,"test"),");"),x(r)}case"DoExpression":{let n=i.call(a=>ft(a,s,e),"body");return x([`do {
`,n.indent(s.tabWidth),`
...
`,i.call(n=>ft(n,s,e),"body").indent(s.tabWidth),`
...
`)},"declaration","decorators"),x(e)}function ft(i,s,e){let t=[];i.each(function(o){let l=o.getValue();l&&(l.type==="EmptyStatement"&&!(l.comments&&l.comments.length>0)||(Z.Comment.check(l)||Z.Statement.check(l)||qr.assert(l),t.push({node:l,printed:e(o)})))});let r=null,n=t.length,a=[];return t.forEach(function(o,l){let c=o.printed,u=o.node,h=c.length>1,f=l>0,d=l<n-1,p,g,y=u&&u.loc&&u.loc.lines,b=y&&s.reuseWhitespace&&On(u,y);if(f)if(b){let P=y.skipSpaces(b.start,!0),k=P?P.line:1,T=b.start.line-k;p=Array(T+1).join(`
`)}else p=h?`
...
`,ys(i).indent(s.tabWidth)])}function zt(i){let s=i.lastPos();do{let e=i.charAt(s);if(/\S/.test(e))return e}while(i.prevPos(s))}function Tn(i){return zt(i)==="}"}function hs(i){return i.replace(/['"]/g,s=>s==='"'?"'":'"')}function dt(i){let s=_e(i,"value"),e=_e(i,"extra");if(e&&typeof e.raw=="string"&&s==e.rawValue)return e.raw;if(i.type==="Literal"){let t=i.raw;if(typeof t=="string"&&s==t)return t}}function ps(i){return JSON.stringify(i).replace(/[\u2028\u2029]/g,function(s){return"\\u"+s.charCodeAt(0).toString(16)})}function _t(i,s){switch(qr.assert(i),s.quote){case"auto":{let e=ps(i),t=hs(ps(hs(i)));return e.length>t.length?t:e}case"single":return hs(ps(hs(i)));case"double":default:return ps(i)}}function ys(i){let s=zt(i);return!s||`
